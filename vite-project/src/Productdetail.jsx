...
const {addBuyToCart}=useCartContext();
  
  const nxtPage = (product) => {
    if (isLoggedIn) {
      navigate(`/account/product/${crit._id}/cart`);  // Use crit._id here
    }
    addBuyToCart(product._id, product.discounted_price, product.title, product.img);
  }
